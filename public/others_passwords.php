...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo '<a class="view_enc_btn" href="view_encryption_process.php?id='.$row['password_id'].'&category='.$row['category'].'&user_id='.$row['user_id'].'&key_id='.$key_id.'">View Encryption Process</a>';
        echo '</div>';
        echo '</div>';
        $pass_num++;
    }
